...
export const VisBeregning = ({ grunnlag }: Props) => {
  ...
  return (
    <>
      ...
      >
        {beregningsVisning === 'STANDARD' && <Grunnlag1119Visning grunnlag={grunnlag.grunnlag11_19} gjeldendeGrunnbeløp={grunnlag.gjeldendeGrunnbeløp} />}
        {beregningsVisning === 'UFØRE' && <UføreVisning grunnlag={grunnlag.grunnlagUføre} gjeldendeGrunnbeløp={grunnlag.gjeldendeGrunnbeløp} />}
        {beregningsVisning === 'YRKESSKADE' && <YrkesskadeVisning grunnlag={grunnlag.grunnlagYrkesskade} gjeldendeGrunnbeløp={grunnlag.gjeldendeGrunnbeløp} />}
        {beregningsVisning === 'YRKESSKADE_UFØRE' && (
          <YrkesskadeUføreVisning grunnlag={grunnlag.grunnlagYrkesskadeUføre} gjeldendeGrunnbeløp={grunnlag.gjeldendeGrunnbeløp} />
        )}
      </VilkårsKort>
    </>
  );
