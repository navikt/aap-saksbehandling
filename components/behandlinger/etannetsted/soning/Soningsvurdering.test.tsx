...
describe('Soningsvurdering', () => {
  ...
  describe("vedleggspanel", () => {
    ...
    test('listen over dokumenter har korrekt beskrivelse', () => {
      expect(
        screen.getByText('Les dokumentene og tilknytt eventuelt dokumenter til 11-26 vurderingen')
      ).toBeVisible();
    });
  ...
  test("Dersom straff sones utenfor fengsel skal begrunnelsestekstfelt vises", async () => {
    const straffUtenforFengselSpærsmål = screen.getByRole("group", {name: "Gjennomfører søker straff utenfor fengsel?"})
    await user.click(within(straffUtenforFengselSpærsmål).getByText("Ja"))

    expect(screen.getByText("Skriv en beskrivelse av hvorfor det er vurdert at søker gjennomfører straff utenfor fengsel")).toBeVisible()
  })
  ...
  test("Dersom søker jobber utenfor anstalt skals begrunnelse-tekstfelt vises", async () => {
    ...
    expect(screen.getByText("Skriv en beskrivelse av hvorfor det er vurdert at søker har arbeid utenforanstalt?")).toBeVisible()
  })
