...
import { Dokument } from "./DokumentTabell"
...
export const DokumentTabellRow = ({dokument, onVedleggClick, onTilknyttetClick}: Props) => {
    return (
        ...
        <Table.DataCell><Link href={`/api/dokument/${dokument.journalpostId}/${dokument.dokumentId}`} onClick={() => onVedleggClick(dokument.journalpostId, dokument.dokumentId)} target="_blank">{dokument.tittel}</Link></Table.DataCell>
        ...
        <Table.DataCell><Checkbox hideLabel checked={dokument.erTilknyttet} onChange={() => onTilknyttetClick(dokument.journalpostId, dokument.dokumentId)}>Tilknytt dokument til vurdering</Checkbox></Table.DataCell>
    </Table.Row>
    )
}